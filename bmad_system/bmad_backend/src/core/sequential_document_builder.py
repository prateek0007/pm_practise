...
class SequentialDocumentBuilder:
    ...
    def __init__(self):
        self.document_templates = {
            ...
            'flf-save': {
                'input_files': [],
                'output_files': ['.sureai/flf-json.txt', '.sureai/flf-mcp-response.txt'],
                'executor': self._execute_flf_save_phase
            }
        }
...
"""
    ...
    def _execute_io8codermaster_phase(self, task_id: str, agent_output: str, project_dir: str, previous_docs: Dict[str, str], agent_prompt: str = "") -> Dict[str, Any]:
        """Execute io8 CodeMaster phase - create breakdown and plan documents"""
        try:
...
"""
            ...
            return {"status": "error", "error": "Failed to execute io8codermaster phase"}
    ...
    def _execute_flf_save_phase(self, task_id: str, agent_output: str, project_dir: str, previous_docs: Dict[str, str], agent_prompt: str = "") -> Dict[str, Any]:
        """Execute FLF Save phase - perform field analysis directly"""
        try:
            ...
            # Execute the FLF agent to perform git operations and field analysis directly
            result = flf_agent.execute(task_id, user_prompt)
            
            # Check if the FLF agent executed successfully
            if result.get('status') != 'success':
                logger.error(f"FLF agent failed: {result.get('error', 'Unknown error')}")
                return result
            
            # The FLF agent has already performed the git operations and field analysis
            # and created the flf-json.txt and flf-mcp-response.txt files
            
            # Get the sureai directory
            sureai_dir = os.path.join(project_dir, ".sureai")
            
            # Check if the expected files were created by the FLF agent
            flf_json_path = os.path.join(sureai_dir, "flf-json.txt")
            flf_mcp_response_path = os.path.join(sureai_dir, "flf-mcp-response.txt")
            
            # Check if the files exist
            if not os.path.exists(flf_json_path):
                logger.warning(f"FLF JSON file not found: {flf_json_path}")
                # Create an empty file as fallback
                with open(flf_json_path, 'w', encoding='utf-8') as f:
                    f.write("[]")
            
            if not os.path.exists(flf_mcp_response_path):
                logger.warning(f"FLF MCP response file not found: {flf_mcp_response_path}")
                # Create an empty file as fallback
                with open(flf_mcp_response_path, 'w', encoding='utf-8') as f:
                    f.write("{}")
            
            # Read the JSON output from the FLF agent
            try:
                with open(flf_json_path, 'r', encoding='utf-8') as f:
                    flf_json_content = f.read()
                
                # Validate that it's valid JSON
                import json
                json.loads(flf_json_content)
                logger.info(f"Successfully read FLF JSON output from {flf_json_path}")
            except Exception as e:
                logger.error(f"Error reading or validating FLF JSON output: {e}")
                flf_json_content = "[]"
                # Save the corrected content
                with open(flf_json_path, 'w', encoding='utf-8') as f:
                    f.write(flf_json_content)
            
            # Read the MCP response
            try:
                with open(flf_mcp_response_path, 'r', encoding='utf-8') as f:
                    flf_mcp_content = f.read()
                logger.info(f"Successfully read FLF MCP response from {flf_mcp_response_path}")
            except Exception as e:
                logger.error(f"Error reading FLF MCP response: {e}")
                flf_mcp_content = "{}"
                # Save the corrected content
                with open(flf_mcp_response_path, 'w', encoding='utf-8') as f:
                    f.write(flf_mcp_content)
            
            # Update the result with the actual file contents
            result['flf_json_content'] = flf_json_content
            result['flf_mcp_content'] = flf_mcp_content
            result['message'] = "FLF field analysis completed successfully"
            
            logger.info("FLF Save phase completed successfully")
            return result
...
"""
            ...
            agent_file_response = self._get_cli_client(task_id, "sm").generate_single_response(create_agent_file_prompt, working_dir=project_dir, agent_name="sm")
...
"""
            ...
            response = self._get_cli_client(task_id, "sm").generate_single_response(create_sm_docs_prompt, working_dir=project_dir, agent_name="sm")
    ...
    def _execute_io8_mcp_project_phase(self, task_id: str, agent_output: str, project_dir: str, previous_docs: Dict[str, str], agent_prompt: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            # Make it absolutely clear what the project name should be with additional emphasis
            project_name_instructions = f"""
CRITICAL PROJECT NAME INSTRUCTIONS:
- The project name MUST be exactly: "{project_name}"
- This is the folder name from the current directory path: "{project_dir}"
...
- Use this exact name: "{project_name}"
...
- THE WORKING DIRECTORY IS SET TO: "{project_dir}" - VERIFY THIS MATCHES THE pwd OUTPUT
"""
...
2. Extract the folder name (last part of the path) - this should be: {project_name}
3. Use this exact folder name "{project_name}" as the project name for io8 project creation
4. DO NOT use generic names like "App" - use the actual folder name: {project_name}
5. Create the io8project only ONCE with the correct name: {project_name}
...
9. CONFIRM THE WORKING DIRECTORY IS SET CORRECTLY TO: "{project_dir}"

Create a base Project Builder plan for the current project. Output to: {builder_file}
Include:
...
- io8 project creation with correct project name: {project_name}
"""
            self._log_prompt_to_gemini("io8_mcp_project_plan", prompt, agent_prompt, previous_docs)
            client = self._get_cli_client(task_id, "io8_mcp_project")
            _ = client.generate_single_response(prompt, working_dir=project_dir, agent_name="io8_mcp_project")
            return {
                'status': 'success',
                'response': 'io8_mcp_project plan created',
                'message': 'io8 MCP Project generated base plan'
            }
        except Exception as e:
